-- ============================================================
-- G&A Property Management — Initial Schema
...
-- Properties: public read, owner/admin can modify
...
-- Units: public read, owner/admin can modify
...
-- Property Images: public read, owner/admin can modify
CREATE POLICY "Property images are publicly readable" ON property_images
  FOR SELECT USING (true);

CREATE POLICY "Owner/admin manage property images" ON property_images
  FOR ALL USING (is_owner_or_admin());
...
-- Auto-Pay: tenants see/manage own, owner/admin sees all
CREATE POLICY "Tenants see own autopay" ON autopay_enrollment
  FOR SELECT USING (tenant_id = auth.uid() OR is_owner_or_admin());

CREATE POLICY "Tenants manage own autopay" ON autopay_enrollment
  FOR INSERT WITH CHECK (tenant_id = auth.uid());

CREATE POLICY "Owner/admin manage autopay" ON autopay_enrollment
  FOR ALL USING (is_owner_or_admin());

-- Maintenance: tenants see/create own, owner/admin sees all
...
-- Maintenance Images: visible if parent request is visible
CREATE POLICY "Maintenance images follow request access" ON maintenance_images
  FOR SELECT USING (
    EXISTS (
...
CREATE POLICY "Owner/admin manage maintenance images" ON maintenance_images
  FOR ALL USING (is_owner_or_admin());
...
-- News: published are public, owner/admin can manage
CREATE POLICY "Published news is public" ON news_updates
  FOR SELECT USING (is_published = true OR is_owner_or_admin());

CREATE POLICY "Owner/admin manage news" ON news_updates
  FOR ALL USING (is_owner_or_admin());

-- Inquiries: anyone can insert, owner/admin can read/manage
...
CREATE POLICY "Owner/admin update inquiries" ON inquiries
  FOR UPDATE USING (is_owner_or_admin());

-- Financial Reports: owner/admin only
CREATE POLICY "Owner/admin see financials" ON financial_reports
  FOR SELECT USING (is_owner_or_admin());

CREATE POLICY "Owner/admin manage financials" ON financial_reports
  FOR ALL USING (is_owner_or_admin());
